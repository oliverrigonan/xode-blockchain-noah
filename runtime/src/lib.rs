...
use configs::{
	xcm_config::{RelayLocation, XcmConfig, XcmRouter, LocationToAccountId},
	RuntimeBlockWeights
};

use xcm::latest::prelude::{Junctions, Junction, Location};
use xcm::{
...
impl_runtime_apis! {
	...
	impl xcm_runtime_apis::fees::XcmPaymentApi<Block> for Runtime {
		...
		fn query_weight_to_asset_fee(weight: Weight, _asset: VersionedAssetId) -> Result<u128, XcmPaymentApiError> {
			let fee: u128 = TransactionPayment::weight_to_fee(weight);

			let numerator: u128 = 1;
			let denominator: u128 = 10_000;

			let converted_fee = fee
